...
- **M5 — People & Roles + Responsibility**: **IN PROGRESS** (spec [docs/M5-people-roles-spec.md](docs/M5-people-roles-spec.md); scope ruled PDL-031). Chosen over the roadmap's "assistant depth", which is Future scope (PDL-031). Three gates: **A ✅ invites/members** (`dd07de1`), B roles+assignments, C responsibility on the card.
  - **Gate A done**: link/token member invites (migration `0009` + `invitations` Edge Function; email delivery deferred, PDL-011), Members list, `/invite` accept flow, People & Roles rail surface (appears after team-flip). Verified: `node scripts/m5-invite-test.mjs` (12/12 live security), `node scripts/m5-invite-walkthrough.mjs` (8/8 two-user browser), 53 unit tests.
  - New primitive: **persisted active org** ([active-org-store.ts](src/modules/organizations/active-org-store.ts)) — a user can belong to several orgs; the stored choice decides the active one (seed of the PDL-009 switcher). Without it an invitee landed in their own personal org.
  - **Edge Functions now**: `classify-capture`, `invitations` (both `verify_jwt=true`; `npm run deploy:function` deploys both and runs the JWT guard on each).
