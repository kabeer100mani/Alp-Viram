...
- **M6 — MVP completion**: **SPEC WRITTEN, awaiting approval — no code.** Renumbered from the roadmap's "Collaboration" (**PDL-038**; the third pre-freeze roadmap correction after PDL-029/PDL-031). Covers the five unbuilt Doc 4 Must Haves — **Tags · org rename · multi-org switcher (PDL-009) · custom saved views · reminder surfacing (PDL-011)** — plus a **TD-006** tag-along (stop displaying the fake "confidence 100%"; keep storing it). Gate A = tags + org rename + switcher (+TD-006); Gate B = custom views + reminders. See [M6 spec](docs/M6-mvp-completion-spec.md).
