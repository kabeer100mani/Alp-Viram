...
- **Permission Model** — **BUILT & VERIFIED, awaiting Palash's gate review.** Migration `0005_permission_hardening.sql` (applied 2026-07-15): access control keyed to the hybrid responsibility model. Closes **TD-001**, **TD-002**, and implements **TDL-012** (the derivation — previously the log's only *Proposed* decision).
  ...
  - Verified: `node scripts/m5-permission-test.mjs` — 21/21, incl. time-bounded revocation and self-assignment escalation. `m1-isolation-test.mjs` still 14/14.
...
- **TD-002** — ✅ **RESOLVED 2026-07-15** (migration `0005`): `activity_events` guarded by a `BEFORE UPDATE/DELETE` trigger (works against `BYPASSRLS`/service role). ⚠️ *Residual:* the service-role path is not exercised by a test (no local service key).
