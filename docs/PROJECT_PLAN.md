...
| **M6** | **MVP completion** | The Doc 4 **Must Haves** still unbuilt: **Tags**, **org rename**, **multi-org switcher** (PDL-009), **custom saved views**, **reminder surfacing** (PDL-011). *(Renumbered 2026-07-16 — PDL-038. Previously read "Collaboration — comments, assignments, realtime": that predates the 2026-07-11 freeze and is contradicted by it — Doc 4 files Comments as Good-to-Have and Real-time as Future; PRD `:143` puts "deep real-time collaboration" in "explicitly NOT in the MVP"; and assignments already shipped in M5.)* |
| **M7** | **Trust & Lifecycle** | The Tier-1 MVP blockers (gap analysis): password reset + minimal email, member offboarding, org deletion (TD-007), snooze wake (TD-011). *(Renumbered 2026-07-16→17 — PDL-040. Previously "Files & notifications — Supabase Storage, attachments, notifications": that predates the freeze and is Future/Good-to-Have — Doc 4 files attachments as Good-to-Have and notification delivery as Future; PRD `:143` puts notification channels in "explicitly NOT in the MVP", PDL-011.)* |
