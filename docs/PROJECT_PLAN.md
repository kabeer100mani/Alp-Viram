...
| **M7** | Files & notifications | Supabase Storage, attachments, notifications. ⚠️ **Same pre-freeze problem as M6 — do not start as written without a PDL:** Doc 4 files "Basic file attachments" as **Good to Have** ("MVP only if cheap") and "Notification delivery" as **Future**; PRD `:143` puts notification delivery channels in "explicitly NOT in the MVP" (PDL-011). |
