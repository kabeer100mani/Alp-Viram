...
### PDL-051 — Home = the task-management view; Capture = a dedicated capture surface (voice + history + clarifying pop-up) (2026-07-19)
- **Decision (Palash, correcting the M8 structure):** **Home** is the **main task-management view** — the item views (Inbox/Today/Upcoming/Aging/Waiting/Done/By Role), the table, Daily Review, Search — and the **default landing**. **Capture** is its **own dedicated surface**: you drop a **message or voice note**, and it shows a **running history/log** of past captures and the task/note each became (with an *optional* "open task" action — never forced). When the AI needs to sort out **type/list/assignee/due**, it asks via a **VS-Code-style approval pop-up** (suggestion + recommendation + an **"add more / elaborate"** field that **re-runs the AI with the extra context**) — the pop-up is *only* the clarifying questions; the input + history live on the Capture page.
- **Reverses PDL-050** (Home is no longer a greeting/overview landing — it IS the task view; the at-a-glance counts survive as a strip atop Home) and **amends PDL-048** (the "Capture" tab held the views — that was backwards; views → Home, and Capture becomes the capture surface). The global **"+" quick capture stays** (reachable from any tab, opening the same clarifying pop-up). My earlier "whole capture as one dialog" was the wrong read — only the *questions* are a pop-up.
- **Nav:** five sections — **Home · Capture · Projects · People · Settings** — with **Statuses folded into Settings** (settings-style customization; keeps the mobile bottom bar at a comfortable 5). Desktop sidebar **collapses/expands on click** (icon-only ⇄ icon+label); mobile keeps the bottom tab bar.
- **Voice notes are in scope:** browser mic recording → a `transcribe` Edge Function (provider-agnostic, Gemini) → the transcript feeds the existing classify pipeline → into the Capture history (audio stored for playback). `ai_captures` already logs every capture, so the history is largely surfacing it.
- **Status:** Accepted (Palash, 2026-07-19). Built as **M9**, gated A (nav restructure) → B (Capture page + clarifying pop-up) → C (voice notes). Spec: [docs/M9-capture-rework-spec.md](../M9-capture-rework-spec.md).
...
- **Relationship to the frozen package:** Doc 5 / the §2 Fable spec fix the status/priority set and colors. This is **additive**: the §2 colors become the **defaults**; customization sits on top per org. Not in the frozen docs — recorded here as the amendment.
- **Why:** teams use their own vocabulary ("Committed" → "Planned", etc.) and colors; it is a common, low-risk personalization that doesn't touch the model.
- **Status:** Accepted (Palash, 2026-07-19). Built in **M8 Gate C** ([spec](../M8-navigation-mobile-spec.md)).

### PDL-048 — Top-level IA becomes five sections (Capture/Projects/People/Settings/Statuses) with a responsive shell; quick capture is a global "+" (2026-07-19)
- **Decision:** The top level moves from Doc 5's **single flat intent-rail** to **five sections** — **Capture · Projects · People · Settings · Statuses** — rendered as a **left sidebar on desktop** and a **bottom tab bar on mobile** (one `sections` config, real routes). The item **views** (Inbox/Today/Upcoming/Aging/Waiting/Snoozed/Done/By Role), **Search**, and **Daily Review** live **inside Capture** as secondary navigation, so **navigation-by-intent is preserved within the work area**. **Quick capture becomes a global floating "+"** (FAB on mobile; top-bar "+"/⌘K on desktop) reachable from every section, replacing the pinned inline capture box.
- **Amends Doc 5 §5/§8** ("navigation is by view/intent, not by tree"; "quick capture in the top bar"): the intent-views are now **nested under Capture** (still intent-based inside), and a **section layer** sits above them; quick capture keeps the principle ("one keystroke from anywhere") but changes placement to the "+".
- **NOT a conflict — implements a frozen intent:** Doc 5 §5 already states *"Mobile is Future; the same views collapse into a **bottom nav** later."* This milestone builds exactly that.
- **Why:** People/Projects/Settings/Statuses don't fit as peers of item-views in one flat rail; and the app was **desktop-only** (verified 2026-07-19) — the restructure is the moment to make it responsive (sidebar ⇆ bottom bar + a content reflow: table → single-column list on mobile).
- **Alternatives:** keep the flat rail + bolt sections on — rejected (doesn't scale, and still desktop-only). A hamburger-only mobile menu — rejected (a bottom bar is the native phone pattern and what Doc 5 anticipated).
- **Status:** Accepted (Palash, 2026-07-19). Built in **M8** ([spec](../M8-navigation-mobile-spec.md)), gated A (nav shell) → B (mobile content reflow) → C (Statuses customization = PDL-049).

### PDL-047 — Due date defaults to "today" (marked "assumed") when none is extracted; confirm-card chips are editable (2026-07-19)
- **Decision:** At capture, when the AI extracts **no due date**, the app **defaults it to today** rather than leaving it blank. A date defaulted this way is flagged **`due_assumed`** and shown with a small **"· assumed" marker** — the **same visual pattern as the "· suggested" list tag** — so it reads clearly differently from a real, user-set date, everywhere it appears (capture card, table row, triage chip). On the **AI confirm card**, **Assignee, Priority, and Due are all editable chips**: Assignee is a dropdown (lists members; solo → just you), Priority is the same `PriorityFlag` control as the table, and Due is a clickable date that shows **today · assumed** by default. **Editing a date clears the assumed flag** (it becomes user-set). Notes carry no due date.
- **Why:** the app sends **no notifications of any kind** (verified — no push/Notification API/scheduled sender); due/remind dates only decide **which view** an item lands in. So a blank due date meant a captured task surfaced in **no dated view** — defaulting to today makes it visible in Today. Separately, the confirm card previously rendered **Priority and Assignee as non-interactive text**, with **nothing shown** when no date was extracted — no way to set them before confirming.
- **How:** migration **`0024`** adds `items.due_assumed` (bool, default false). `createItem` sets it; `updateItem` **clears it whenever the user explicitly sets `due_at`**. `DateCell` gained an `assumed` prop rendering the "· assumed" tag; the confirm card computes today-at-local-midnight (the "no specific time" convention, PDL-043) when the AI gave nothing.
- **Trade-off (heads-up):** every dateless capture now carries a *today* due date, so more items surface in **Today** — but they are visibly tagged "· assumed", so a real deadline still stands out. Tunable later if it proves noisy.
- **Status:** Accepted (Palash, 2026-07-19). Verified live: no-date capture → card shows "today · assumed" with editable Priority/Assignee/Due; the row carries the "· assumed" marker; editing the date clears `due_assumed`. 114 unit tests green.

### PDL-046 — Assignee is mandatory and never blank; silently defaults to the creator (2026-07-18)
- **Decision:** Every **Task** carries an **assignee**, and it is **never blank** — at creation it **silently defaults to the item's creator** (exactly like the "General" List default: zero friction, no question). It is shown as an **already-filled, editable chip** in the AI confirmation card (same treatment Priority gets); tap to reassign to another member, otherwise it stays the creator. **Notes carry no assignee** (responsibility is a task concept — a Note is a reference, not assigned work).
- **Reverses** the earlier "don't ask/attach assignee at capture" stance (Doc 4's zero-field capture; PDL-021 keyed write to the *creator* and never assigned anyone) — **by Palash's explicit choice** (2026-07-18). A superseded intermediate idea — a capture-time **"Who's this for?"** follow-up mirroring the List follow-up — was **dropped as the wrong pattern**: no question is asked; the creator default just fills it.
- **Why:** a field audit found this was a **genuine gap** — `createItem` inserted only the item row and never wrote `item_assigned_users`, so **every captured task had no owner** ("UNFILLED — needs owner"). Mandating a creator default closes it with no added friction.
- **How:** `createItem` inserts the item, then inserts the assignee (chosen or creator) as the **primary** `item_assigned_users` row (`is_primary`). The creator has write at that moment (creator axis of `can_write_item`), so the insert passes RLS.
- **Correction (Palash, 2026-07-19): Assignee is ALWAYS shown — solo and team alike.** It is a **mandatory per-item field like Priority/Due date**, so **PDL-022 does NOT apply to it** (PDL-022 hides *team/role administration*, not a per-item field). The **Assignee column shows for everyone** (a solo user sees their own avatar — the auto-assigned creator), and the **capture card always shows the assignee chip**. Reassignment is offered **only once other people exist** (a team → dropdown picker); a solo user sees a **static chip** of themselves. *(This reverses this PDL's first-cut "no chip for solo / column team-only" — that wrongly treated assignee as a PDL-022 concept.)*
- **Alternatives:** (a) a DB trigger auto-assigning the creator on every `items` insert — rejected for now: it would fight the "reassign before confirm" flow and is a migration on the permission-sensitive responsibility model; the write-path default covers all app creation. (b) the "Who's this for?" capture question — rejected (adds friction; a silent default is zero-friction).
- **Trade-offs:** the "UNFILLED assignee" state no longer occurs for tasks (responsible *roles* can still be unfilled). A solo user sees the assignee concept only if/when they add a team. The invariant is enforced at the **write path**, not the DB (a raw insert could still skip it — acceptable; tests do this deliberately).
- **Status:** Accepted (Palash, 2026-07-18). Verified: `scripts/m14-assignee-test.mjs` (task auto-assigns creator as primary; explicit assignee honoured; Note gets none) + AiCaptureBox unit test asserts the default.

### PDL-045 — Self-service workspace creation; solo users get a "New workspace" entry (relaxes PDL-022 one notch) (2026-07-18)
- **Decision:** A signed-in user can create **additional personal Organizations of their own** — "Personal", "Job 1", "Job 2" — from a **"New workspace"** entry (name field), landing straight in the new empty org. Each may be **solo** (no members required); the PDL-009 switcher already lists them all by name. Previously an org was only ever obtained by **sign-up** (one automatic personal org) or by **invitation** into someone else's — there was no create path at all.
- **Why:** Palash wants Organizations usable as personal "profiles" with separate Projects/Folders/Lists, switched between at will. The switcher half already existed; only self-creation was missing.
- **How:** A **`SECURITY DEFINER` routine `create_organization(name)`** (migration `0022`), not two client inserts — `orgs_insert` lets any authed user create the org row, but `members_insert` requires `is_org_admin`, and the creator is not yet a member of the brand-new org, so the owner-membership insert is RLS-refused (same reason the sign-up bootstrap is `SECURITY DEFINER`). Migration `0023` additionally teaches `protect_owner_membership` to allow seeding the **first owner of an owner-less org** (bootstrap) while still blocking a non-owner from grabbing owner in a **live** org — a live org can never reach zero owners (last-owner rule), so there is **no escalation regression** (verified: m11-offboarding / m5-permission / m1-isolation all still green). System views seed automatically (`trg_seed_system_views`, 0007); the org stays `is_personal` (single member).
- **PDL-022 relaxation:** progressive disclosure hides the workspace name / one-entry switcher from a solo user. Self-creation requires an entry point they *can* see, so a **quiet "New workspace" affordance** now shows for a solo+single-org user (no name, no one-item switcher). The full **named** switcher still only appears once there are **2+** orgs. This is the minimal surface that makes the feature reachable.
- **Alternatives:** (a) Loosen `members_insert` RLS to let a creator self-insert owner — rejected: weakens a broad policy for one flow; the `SECURITY DEFINER` routine is narrower and mirrors existing precedent. (b) Put "New workspace" in an account menu instead of the switcher — rejected: no such menu exists, and Palash asked for it "in the switcher".
- **Trade-offs:** A solo user now sees the workspace concept slightly earlier than PDL-022 intended; accepted as the deliberate cost of the feature. No member-management is implied (still solo unless invited).
- **Status:** Accepted (approved by Palash, 2026-07-18). Verified: `scripts/m13-create-org-test.mjs` 11/11 live (bootstrap complete, owner, 8 system views, empty, + the RLS gap the routine bridges) · 6 org-repo unit tests · security regressions green.

### PDL-044 — Free-text Project context feeds AI *suggestions* (user-confirmed), built after the List follow-up (2026-07-18)
- **Decision:** A **free-text context** field on Projects (what it's about / who's involved) that the AI uses to make **smarter suggestions** — **ranking which List** a capture belongs to and **who's responsible** — **which the app presents as options the user taps to confirm. No silent AI filing** (D1). Scoped to the honest, minimal v1 (see rulings); full structured per-project teams/roles and all-projects-at-capture are **out of scope**.
- **Rulings (Palash, 2026-07-18):**
  - **D1** — allow **context-driven suggestions** (list ranking + responsibility), but the **app owns the options and the user taps to confirm**; no silent AI filing. This **amends PDL-032** ("the AI does not infer the list"): the AI may now **rank the org's real lists** as a suggestion, but never auto-files — the human still chooses, matching the PDL-042 "app owns the options" pattern.
  - **D2** — **free-text** project context for now, **not** structured team/role data (avoids the org-scoped-roles conflict; project-level roles stay Future).
  - **D3** — **parallel, not sequential:** fund + run the **Anthropic accuracy batch** alongside this build (not as a gate before it). *(Funding is Palash's action; the batch runs once the account has credits — set `AI_PROVIDER=anthropic`, `npm run deploy:function`, `node scripts/m3-classify-test.mjs`, and fix TD-012 then.)*
  - **D4** — **sequencing:** finish the **List follow-up (PDL-042) first**; Project-context suggestions are the **second layer on top** of it.
- **Architecture (from the plan):** use a project's context **only where a project is known** (Daily Review filing / after the follow-up narrows to a project) — **one** project's context per call (bounded token cost, privacy-respecting), **never** all-projects-at-capture. Extends the provider-agnostic, Zod-validated classifier contract (input: optional context; output: a role/list-ranking suggestion) and redeploys the Edge Function; stays "AI proposes, user confirms" (PDL-012).
- **Amends the frozen package:** pulls a slice of "**Advanced AI / smart suggestions**" (PRD §7 / Doc 4 Future; PDL-031 deferred) forward — ruled acceptable by Palash, run in parallel with de-risking the AI foundation via the Anthropic batch (D3). Structured per-project roles remain **Future** (DB design; Doc 4 "role inheritance / fine-grained visibility").
- **Status:** Accepted (ruled by Palash, 2026-07-18). **v1 BUILT (keyword-first list ranking); responsibility-suggestion deferred to a later increment** (needs a role field in the AI contract + Daily Review UI). Build order followed: time → List follow-up → this.
- Full scoping + citations: [docs/plan-project-context-ai.md](../plan-project-context-ai.md); keyword-vs-AI addendum ruled keyword-first.
