...
- **How:** `createItem` inserts the item, then inserts the assignee (chosen or creator) as the **primary** `item_assigned_users` row (`is_primary`). The creator has write at that moment (creator axis of `can_write_item`), so the insert passes RLS. **Solo users:** silent creator default, **no chip** (a solo user has no one else to assign to, and responsibility stays hidden — **PDL-022**). **Team orgs:** the editable assignee chip appears in the capture card.
